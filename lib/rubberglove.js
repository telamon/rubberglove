...
module.exports.firewall={
	...
	addRule: (opts) => {
		...
		if(opts.match.program)		instr.push(`-Program "${opts.match.program.replace(/\//g,"\\")}"`);
...
module.exports.services={
	...
	*/
	...
	start: (id)=>{
		assert.ok(id,'Id(Name) of service is required.');
		return execPS(`Set-Service -Name ${id} -Status Running -PassThru`,{jsonDepth: 3})
		.then((res)=>remapService(res));
	},
	start: (id)=>{
		assert.ok(id,'Id(Name) of service is required.');
		return execPS(`Set-Service -Name ${id} -Status Stopped -PassThru`,{jsonDepth: 3})
		.then((res)=>remapService(res));
	}
...
*/
function remapApp(app){
	app.id=app.Name;
	return Object.keys(app).reduce((a,key)=>{
		let original_key = key;
		key = key.split('');
		debugger;
		key = key.splice(0,1)[0].toLowerCase() + key.join('');
		a[key] = app[original_key];
		return a;
	},{})
}
module.exports.app={
	list: (search)=>{
		...
		return execPS(`Get-AppxPackage ${search}`)
		.then((res)=>{
			return res.map((app)=>remapApp(app));
		});
	}
}