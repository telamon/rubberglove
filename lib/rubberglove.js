...
module.exports.registry={
	get:function(key){
		...
		return execPS(`Get-ItemPropertyValue -Path ${rpath} -Name ${name}`);
	},
...
const serv = {
	remapService: (s)=>{
		return {
			id: s.Name,
			name: s.DisplayName,
			startType: ([,,'automatic','triggered','disabled'])[s.StartType],
			status: ([,'stopped',,,'running'])[s.Status],
			dependsOn: s.RequiredServices,
			dependencyOf: s.DependentServices
		};	
	}
};
module.exports.services={
	list:(opts)=>{
		...
		return execPS(`Get-Service ${params.join(' ')}`+ (where.lenght? `| Where ${where.join(' ')}`:''),{jsonDepth: 2})
		.then((res) => {
			return res.map((service) => serv.remapService(service));
		});
	}
}